...
String FName = "20220122.txt";     //USER: SD card file name (ANSI encoding is best)
String SDData;                     //Build data to write to SD "disk"
String value;                      //Create Temp string to write to SD
...
void getRecordFile() {             //Read from SD file
  if (dataFile) {                  //If file is there
    Serial.write(dataFile.read()); //Read, then write to COM
...
void loop(){                       //LOOP()
  ...
  value = String(Dh)+','+String(pressure)+','+String(p0*100)+','+String(cTemp);  //SD data
